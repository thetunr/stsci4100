...
setwd("~/Documents/06.sp25/04.stsci4100/stsci4100") # DELETE UPON SUBMISSION
number.of.rows = -1 # Value of -1 will read all rows
# nrows = 200000 # Uncomment to limit number of rows read from dataset

import.data <- FALSE
if (import.data) {
  basics <- fread("title.basics.tsv", sep = "\t", 
                  na.strings = "\\N", nrows = number.of.rows)
  crew <- fread("title.crew.tsv", sep = "\t", 
                na.strings = "\\N", nrows = number.of.rows)
  principals <- fread("title.principals.tsv", sep = "\t", 
                      na.strings = "\\N", nrows = number.of.rows)
  ratings <- fread("title.ratings.tsv", sep = "\t", 
                   na.strings = "\\N", nrows = number.of.rows)
}

save.data <- FALSE
if (save.data) {
  saveRDS(basics, "basics.rds")
  saveRDS(crew, "crew.rds")
...
read.data <- TRUE
if (read.data) {
  basics <- readRDS("basics.rds"); 
  crew <- readRDS("crew.rds"); 
  principals <- readRDS("principals.rds"); 
...
+ **tconst**: There are no missing values. There are exactly 11,544,906 id's (i.e. individual films), one for every row.
+ **titleType**: There are no missing values. There are 11 unique kinds of films in this dataset, ranging from shorts to video games.
+ **primaryTitle**: There are no missing values. There are 5,191,359 primaryTitles in the dataset (< 11,544,906). This should be looked into.
+ **originalTitle**: There are no missing values. There are 5,217,113 originalTitles in the dataset (< 11,544,906 and > 5,191,359). This should be looked into.
+ **isAdult**: There are no missing values. Approximately 3.23% of all the films in the dataset are adult films. 
+ **startYear**: There are missing values! Ignoring missing values, the earliest release date was 1874, while the latest release date was 2031. 
+ **endYear**: There are missing values! Ignoring missing values, the earliest end date was 1928, while the latest end date was 2030.
+ **runtimeMinutes**: There are missing values! Ignoring missing values, the shortest film is 0 minute long, while the longest film is 3,692,080 minutes (61,534 hours 40 minutes) long. This longest runtime seems unusual. The average film length is around 44.38 minutes (0.74 hours).
...
+ **job**: There are missing values — this is because of the duplicate values from the first column. There are 44,351 unique values within this column. 
+ **characters**: There are missing values — this is because of the duplicate values from the first column. There are 4,251,944 unique values within this column. 
...
+ **numVotes**: There are no missing values. Of all 11,544,906 films, 513 films have received the lowest number of reviews (only 5 reviews). Of all 11,544,906 films, only one film (i.e. the film in the 84,693rd row) has the largest number of reviews (3,021,924 reviews).
...
dim(basics)                                        # This is correct — it should be (11,544,906 rows, 9 columns).
dim(crew)                                          # This is correct — it should be (11,544,906 rows, 3 columns).
dim(ratings)                                       # This is interesting — it expect it to be (11,544,906 rows, 3 columns).
...
2. However, our final dataset does **not** contain 11,544,906 rows. It has the right number of columns, but it seems that we are bottlenecked by the number of rows in ratings. 
