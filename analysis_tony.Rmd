...
crew data but some are missing from `full_df`.
...
TODO: EXPERIMENTING QUESTION 1

```{r}
library(data.table)

# Clean data
valid <- full_df$numVotes > 0 & !is.na(full_df$averageRating)
dt <- data.table(
  votes = full_df$numVotes[valid],
  rating = full_df$averageRating[valid],
  x_log = log10(full_df$numVotes[valid])
)
...
# Sample up to 100 per bin + compute stats
dt_sampled <- dt[, {
  idx <- sample(.N, min(.N, 100))
  .SD[idx]
}, by = bin]

# Summary stats for IQR ribbon
summary_stats <- dt[, .(
  p25 = quantile(rating, 0.25),
  p75 = quantile(rating, 0.75),
  mean_rating = mean(rating),
  votes_bin = 10^mean(x_log)  # use midpoint for plotting
), by = bin][order(votes_bin)]

# Base plot setup
plot(NA,
     xlim = range(summary_stats$votes_bin),
     ylim = range(dt_sampled$rating, na.rm = TRUE),
     log = "x",
     xlab = "Number of votes",
     ylab = "Average rating",
     main = "Average Rating vs. Number of Votes (with spread)")

# Add 25–75% shaded ribbon
polygon(c(summary_stats$votes_bin, rev(summary_stats$votes_bin)),
        c(summary_stats$p25, rev(summary_stats$p75)),
        col = rgb(135, 206, 250, 100, maxColorValue = 255), border = NA)

# Add mean line
lines(summary_stats$votes_bin, summary_stats$mean_rating, col = "deeppink3", lwd = 2)

# Add sampled points (actual ratings)
points(dt_sampled$votes, dt_sampled$rating, pch = 16, col = rgb(0, 0, 0, 40, maxColorValue = 255), cex = 0.6)

# Legend
legend("bottomright",
       legend = c("Sampled ratings", "Mean per bin", "25–75% range"),
       col = c("black", "deeppink3", rgb(135, 206, 250, maxColorValue = 255)),
       pch = c(16, NA, 15), lwd = c(NA, 2, NA), pt.cex = 1.5)
```
...
# [2] Merge the new DataFrame with full_df's "tconst" and "averageRating" column.
problem3 <- merge(actors.dataset,                                                            # New DataFrame we'll use for Problem 3
                  full_df[, c("tconst", "averageRating")],                                # 7 columns: same as principals, except with the column averageRating added.
                  by = "tconst")
...
+  Unlike the main dataset full_df, we're no longer dealing with one row for one movie.
