...
server <- function(input, output, session) {
  ...
  # Reactive expression to filter data based on genre and start year
  ...
  # Render the scatter plot with the chosen regression method
  ...
  # Render a summary of the regression analysis
  output$summaryText <- renderPrint({
    dt <- filteredData()
    if (nrow(dt) < 2) {
      cat("Not enough data to perform analysis.")
    } else {
      if (input$regressionMethod == "LOWESS") {
        lw <- lowess(dt$numVotes, dt$averageRating)
        cat("LOWESS Smoother:\n")
