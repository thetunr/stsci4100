...
# COMBINING DATASETS:
# [1] Combining basics, crew, and ratings into one data frame. 
full_df <- basics[crew, on = "tconst"]
full_df <- full_df[ratings, on = "tconst"]
# View(full_df) # Uncomment to view data frame
...
dim(full_df)  # This is interesting — we expected (1,568,333 rows, 13 columns). 
...
2. However, our final dataset contains 1,568,336 rows rather than 1,568,333
rows. It has the right number of columns, but it seems that we are **not** being
bottlenecked by the number of rows in `crew`. This may be a problem later when
we try to do analysis on the crew data but some are missing from `full_df`.
...
length(unique(principals$tconst)) # 1542357 — This seems like the real bottleneck. (It's the smallest number.)  
...
rows for each `tconst` value into one row in our final data frame. 
...
# DIAGNOSTICS: [PART 4]
full_df <- basics[crew, on = "tconst"]
full_df <- full_df[principals, on = "tconst"]
full_df <- full_df[ratings, on = "tconst"]
nrow(full_df)
# saveRDS(full_df, "full_df.rds") # Run this line to save this cleaned df
```
...
# [3] DATA ANALYSIS:
Now that we have gone through the datasets and obtained a final data frame 
`full_df` to work with, we'll tackle **five questions** starting with the most 
intuitive one first.
...
```{r q1-5}
full_df$votecategories <- NA                                                                                    # Create a new column in full_df.
full_df$votecategories[full_df$numVotes <= 250000] <- "Extremely Low (<250K)"                                # Category 1: Extremely Low
full_df$votecategories[full_df$numVotes > 250000 & full_df$numVotes <= 500000] <- "Low (251K–500K)"       # Category 2: Low
full_df$votecategories[full_df$numVotes > 500000 & full_df$numVotes <= 750000] <- "Medium (501K–750K)"    # Category 3: Medium
full_df$votecategories[full_df$numVotes > 750000 & full_df$numVotes <= 1000000] <- "High (751K-1M)"       # Category 4: High
full_df$votecategories[full_df$numVotes > 1000000] <- "Extremely High (>1M)"                                 # Category 5: Extremely High

full_df$votecategories <- factor(full_df$votecategories, levels = c("Extremely Low (<250K)",
                                                                          "Low (251K–500K)",
                                                                          "Medium (501K–750K)",
                                                                          "High (751K-1M)",
                                                                          "Extremely High (>1M)"))
...
full_df$haswriter <- as.numeric(!is.na(full_df$writers))                  #  Binary column: 1 if there's a writer, 0 otherwise.

# [2] Obtain a summary statistic and mean for the column haswriter.
summary(full_df$haswriter)
mean(full_df$haswriter)
...
summary(full_df$averageRating)
mean(full_df$averageRating)
...
boxplot(averageRating ~ haswriter, data = full_df,
        col = c("red", "blue"),
...
t.test(averageRating ~ haswriter, data = full_df)
```
...
To answer this question, we need to realize that we only care about the following columns within the dataset `full_df`:
...
Let's first filter out our dataset `full_df`. We only want the rows where the **category** column contains actor information.
```{r q2-2}
# [1] Create a new dataset called actors.dataset. It's a filtered version of full_df, only containing the rows that say "actor" and "actress."
actors.dataset <- full_df[grepl("actor|actress", principals$category), ]
...
Let's go over some important caveats to notice:
...
library(data.table)                                                             # Load data.table library — used to handle large datasets.
setDT(actors.dataset)                                                           # Convert to data.table.
long <- actors.dataset[, .(nconst = unlist(nconst)), by = .(averageRating)]     # Use unlist() function to unlist each entry — ensures each actor gets their own row.

# [2] Compute each actor's mean rating and number of films.
actor.results <- long[, .(                                                      # Defines a DataFrame made of two columns — "average ratings" and "number of films" for each actor.
  average.ratings = mean(averageRating, na.rm = TRUE),
  number.of.films = .N
), by = nconst][order(-average.ratings)]                                        # Using order(-average.ratings) will order our results from most to least film average ratings.

head(actor.results)                                                             # Displays head of DataFrame.
tail(actor.results)                                                             # Displays tail of DataFrame.
```

As we can see, when we rearranged our DataFrame to show us the actors who starred in films that got the highest to least average ratings, we ended up mostly seeing actors who starred in only one film at the top. This tells us something important:

+ Many actors who starred in films with high ratings (like 10/10) were actors who, **at least according to our dataset**, starred in only one film.
+ That's not that telling.
+ Why? Because an actor who only starred in one film that got 10/10 reviews will obviously have an average film rating of 10/10. That clearly doesn't say much in our analysis.
+ On the other hand, an actor who starred in say 1000 films that all got 10/10 reviews means significantly mean more.
+ Yes, it could be true that if an actor starred in **many** films that have an overall high rating, then the actor had some influence on the rating.
+ However, if the actor has only appeared in **only one** film (and that film has a high rating), then it's harder to reach those same conclusions.

So, let's see what happens if we focus on actors who starred in, say for now, at least 5 films.
```{r q3-2}
...
unique.length <- length(unique(full_df$genres))                               # Defines the length of the unique entries in the column "genres."
#anyNA(full_df$genres)                                                        # Remember there are missing values in the column "genres."

list.of.genres <- list()                                                         # Instantiates an empty list.
for (i in 1:unique.length){                                                      # Uses a for-loop to update the empty list.
  extracted.vector <- strsplit(unique(full_df$genres)[i], split = ",")[[1]]
...
number.of.shorts <- length(which(full_df[,9] == "Short"))               # Number of rows in dataset that just say "Short."
number.of.NA <- sum(is.na(full_df[,9]))                                 # Number of rows in dataset that have missing values.
...
for (i in unique(full_df$titleType)){                                          # [1] For each kind of film, compute the average ratings for its pure genres.
  ...
  movie.indices <- which(full_df$titleType == i)
  
  for (j in pure.genres){                                                         # Update both empty vectors.
    indices <- which(grepl(j, full_df$genres[movie.indices]))
    how.many.ratings <- length(indices)
    
    ratings <- full_df$averageRating[indices]
    number.of.votes <- full_df$numVotes[indices]
...
hist(full_df$averageRating, col = "azure1")
summary(full_df$averageRating)
```
