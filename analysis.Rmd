...
```{r}
basics <- readRDS("rds/basics.rds"); 
crew <- readRDS("rds/crew.rds"); 
principals <- readRDS("rds/principals.rds"); 
ratings <- readRDS("rds/ratings.rds"); 
...
for (i in 1:4) print(paste("There are", nrow(list.of.datasets[[i]]), "rows in the", names(list.of.datasets)[i], "dataset."))
```
...
save.my.dataset = F
if (save.my.dataset) { saveRDS(my.dataset, "rds/mydataset.rds") }
...
dim(my.dataset)                                    # This is WRONG — it should be (200000 rows, 13 columns). But instead, it has 139602 rows (SOME ARE MISSING).
```
...
# Here, the mean of the column haswriter is 0.62782 This means that roughly 63% of our films in the dataset have a specified writer.
...
3. Just to be safe, we conducted a two-sample t-test. This is where things got interesting:
   ...
   + In addition, the t-test's results shows us that the mean for "No writer" is 6.320407 while the mean for "Writer" is 6.148094. Those means are incredibly close and identical.
