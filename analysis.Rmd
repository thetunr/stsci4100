...
dim(full_df)     # This is interesting — we expected (1,568,333 rows, 13 columns), not 1,568,336 rows.
```
...
rows for each `tconst` value into one row in our final dataframe. 
...
# [3] DATA ANALYSIS:
Now that we have gone through the datasets and obtained a final dataset **full_df** to work with, we'll tackle **five questions** starting with the most intuitive one first.
