...
## D. Combining the Datasets.
Excluding the principals dataset, let's combine the three datasets that share 
the same first column **tconst** (i.e. the same unique ID values):
...
```{r}
# EXTRA STEP IN SUBSETTING:
# Subsetting basics and ratings by crew. Crew has less rows than basics and 
# crew, and to avoid any mismatching tconst values, we subset those rows. 
crew_tconsts <- unique(crew$tconst)
basics     <- basics[tconst %in% crew_tconsts]
principals <- principals[tconst %in% crew_tconsts]
ratings    <- ratings[tconst %in% crew_tconsts]
# Now, basics, crew, ratings should all have the same number of rows. 
```
...
# View(my.dataset) # Uncomment to view dataset

# [2] Checking the dimensions (rows, col).
dim(basics)      # This is correct — it should be (1,568,333 rows, 9 columns).
dim(crew)        # This is correct — it should be (1,568,333 rows, 3 columns).
dim(ratings)     # This is correct — it should be (1,568,333 rows, 3 columns).
dim(my.dataset)  # This is correct — it should be (1,568,333 rows, 13 columns).
```
