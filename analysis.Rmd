...
**TO DO: EXPLAIN THIS CODE CHUNK IN WORDS**
...
**TO DO: PICK AND CHOOSE VALUES FROM THE SUMMARY. PLEASE DON'T INCLUDE THE WHOLE SUMMARY**
...
**TO DO: DON'T INCLUDE WHOLE SUMMARY**
...
To investigate this catch, we decided to **deliberately** investigate other extremely famous actors who we know also starred in many movies. Specifically, we investigated the following actors (with their IDMb ID's obtained from a quick **Google search**):
...
```{r q3-8, echo = FALSE}
top10_movies[, .(primaryTitle, averageRating, numVotes)]
```
...
```{r q4-3, fig.width = 10, fig.height = 7, echo = FALSE}
par(mfrow = c(2, 5))
...
# CONTRIBUTIONS:
We worked on the whole file together, going back and forth between workflows.

+ Tony: Questions 1, 2, 5
+ William: Questions 3, 4, 5
